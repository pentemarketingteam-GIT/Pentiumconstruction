...
import { BRAND, STATS, PROJECTS, IMAGES } from "@/lib/pentium";
...
const tickerItems = [
  ...
  "Spring Green Villas, Perinthalmanna — Delivered",
  "RERA Certified — All Projects",
  "Pentium Tranquil — Ready to Move",
  "Pentium Eternia — Delivered",
];
    ...
    <div className="min-h-screen bg-pen-mesh" style={{ color: "var(--pen-ink)" }}>
      ...
      <header className="sticky top-0 z-40 flex h-[68px] items-center justify-between px-5 sm:px-10" style={{ background: "var(--pen-bg-top)", backdropFilter: "blur(16px)", borderBottom: "1px solid var(--pen-border)" }}>
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-display text-lg">Pentium <span className="pen-gold-text">Constructions</span></span>
        </div>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0" style={{ background: "linear-gradient(90deg, rgba(11,14,19,0.94) 0%, rgba(11,14,19,0.82) 45%, rgba(11,14,19,0.55) 100%)" }} />
        </div>
        <Particles count={16} />
        <div className="relative z-10 mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-10 lg:grid-cols-[1.1fr_0.9fr] lg:py-28">
          <div>
            <motion.span initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-[11px] font-semibold uppercase tracking-[0.2em]" style={{ border: "1px solid var(--pen-border)", background: "var(--pen-surface)", color: "var(--pen-gold-2)" }}>
              <ShieldCheck className="h-3.5 w-3.5" /> Premium Residential · Calicut, Kerala
            </motion.span>
            <motion.h1 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mt-5 font-display text-5xl leading-[1.05] sm:text-6xl">
              Homes built to <span className="pen-serif pen-gold-text">last for generations</span>
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }} className="mt-5 max-w-xl text-lg" style={{ color: "var(--pen-fg-2)" }}>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.26 }} className="mt-8 flex flex-wrap gap-3">
              ...
              <a href={BRAND.phoneHref} className="pen-btn-ghost"><Phone className="h-4 w-4" /> Call us</a>
              <a href={BRAND.whatsapp} target="_blank" rel="noreferrer" className="pen-btn-ghost"><MessageCircle className="h-4 w-4" /> WhatsApp</a>
            </motion.div>
            <div className="mt-12 grid max-w-xl grid-cols-4 gap-3">
              {STATS.map((s) => (
                <div key={s.label}>
                  <p className="font-display text-2xl sm:text-3xl" style={{ color: "var(--pen-gold-2)" }}>{s.noPlus ? s.value : s.value.toLocaleString()}{s.suffix}</p>
        ...
        {/* ticker */}
        <div className="relative z-10 overflow-hidden py-3" style={{ borderTop: "1px solid var(--pen-border)", borderBottom: "1px solid var(--pen-border)", background: "var(--pen-bg-top)" }}>
          <div className="flex whitespace-nowrap" style={{ animation: "pen-marquee 26s linear infinite" }}>
            {[...tickerItems, ...tickerItems].map((t, i) => (
              <span key={i} className="mx-6 inline-flex items-center gap-2 text-sm" style={{ color: "var(--pen-fg-2)" }}>
                <span className="h-1.5 w-1.5 rounded-full" style={{ background: "var(--pen-gold)" }} /> {t}
              </span>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-10">
        <div className="flex items-end justify-between">
          <div>
            <span className="font-display text-[11px] font-semibold uppercase tracking-[0.22em]" style={{ color: "var(--pen-gold)" }}>Our Portfolio</span>
            <h2 className="mt-1.5 font-display text-3xl sm:text-4xl">Latest projects</h2>
          </div>
            ...
            <motion.div key={p.key} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: (i % 3) * 0.06 }}>
              <Link to="/advisor" className="group flex h-full flex-col overflow-hidden rounded-3xl" style={{ border: "1px solid var(--pen-border)", background: "var(--pen-surface)" }}>
                <div className="relative h-44 overflow-hidden">
                  ...
                  <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, transparent 40%, rgba(11,14,19,0.9))" }} />
                  <span className="absolute left-3 top-3 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-wide" style={{ background: p.status === "ongoing" ? "rgba(201,166,98,0.2)" : "rgba(111,191,139,0.18)", color: p.status === "ongoing" ? "var(--pen-gold-2)" : "var(--pen-ok)", border: "1px solid var(--pen-border)" }}>{p.badge}</span>
                </div>
      ...
      {/* mobile enquiry */}
      <section className="mx-auto max-w-2xl px-5 pb-20 lg:hidden">
      ...
      <section className="relative overflow-hidden py-16" style={{ borderTop: "1px solid var(--pen-border)" }}>
        <div className="mx-auto max-w-3xl px-5 text-center">
          <h2 className="font-display text-3xl sm:text-4xl">Ready to own your dream home in Kerala?</h2>
